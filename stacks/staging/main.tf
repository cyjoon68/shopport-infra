terraform {
  ...
  backend "s3" {
    key          = "shopport/staging/terraform.tfstate"
...
provider "aws" {
  ...
  default_tags { tags = { Application = "shopport", Environment = "staging", ManagedBy = "terraform" } }
}

resource "aws_route53_zone" "environment" {
  count = var.subdomain_zone_name == "" ? 0 : 1
  name  = var.subdomain_zone_name
  tags  = { Application = "shopport", Environment = "staging", ManagedBy = "terraform" }
}
